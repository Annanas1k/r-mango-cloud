...
export const TrashPage = () =>{
    ...
    return (
        ...
        {trashItems.length === 0 ? (
          ...
          <div className="w-full h-full flex flex-wrap gap-4 bg-gray-50">
            {trashItems.map((item)=>(<TrashCards key={item.id} item={item} isSelected={selectedId === item.id} onSelect={handleSelect} onOpen={handleOpen} />))}
          </div>
        )}
            </section>
        </main>
